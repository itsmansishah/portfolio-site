...
export function navigate(to: string) {
  ...
  if (hash) {
    ...
    requestAnimationFrame(() => {
      ...
      if (el) el.scrollIntoView({ behavior: targetPath === window.location.pathname ? "smooth" : "auto" });
  ...
  return (
    <nav className="sticky top-0 z-30 bg-paper/85 backdrop-blur-sm">
      <div className="mx-auto flex max-w-[1100px] items-center justify-between px-6 py-5">
        ...
        <Link to="/#work" className={`${LABEL} hidden hover:opacity-60 sm:block`}>
          Work
        </Link>
        <Link to="/#about" className={`${LABEL} hover:opacity-60`}>
          About Me
        </Link>
      </div>
    </nav>
