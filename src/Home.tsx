...
function Hero() {
  ...
  return (
    <header
      id="top"
      className="mx-auto grid max-w-[1100px] items-end gap-8 px-6 pt-8 dt:grid-cols-12 dt:gap-10 dt:pt-16"
    >
      ...
      <div className="dt:col-span-7">
        ...
        <p
          className="rise mt-2 text-right font-serif text-[clamp(3.25rem,16vw,3.75rem)] leading-[1.02] tracking-[-0.03em] dt:text-left dt:text-[clamp(1.6rem,7vw,3rem)] dt:leading-[1.08]"
          style={delay(120)}
        >
        ...
        <p
          className="rise ml-auto mt-5 max-w-[34rem] text-right font-mono text-[12px] font-medium uppercase leading-[1.7] tracking-[0.08em] dt:ml-0 dt:mt-6 dt:text-left dt:text-[11px] dt:tracking-[0.18em]"
          style={delay(200)}
        >
      ...
      <div className="dt:col-span-5">
        ...
        <div className="relative mx-auto mt-6 w-full max-w-[420px] dt:ml-auto dt:mr-7 dt:mt-0 dt:w-[min(88%,340px)]">
          <div
            aria-hidden="true"
          ...
          <a
            ...
            className={`${LABEL} rise fixed bottom-[max(1.25rem,env(safe-area-inset-bottom))] right-5 z-40 whitespace-nowrap rounded-full bg-accent px-5 py-3 text-ink shadow-md transition hover:brightness-95 dt:absolute dt:bottom-5 dt:right-4 dt:z-auto`}
            style={delay(420)}
          >
            Say hello
          </a>
...
function ProjectRow({ project }: { project: Project }) {
  return (
    <article className="border-t border-ink/15 py-8 dt:py-14">
      <div className="grid gap-5 dt:grid-cols-12 dt:gap-10">
        ...
        <div className="dt:col-span-5">
          ...
          <div className="flex items-baseline justify-between gap-4 dt:justify-start dt:gap-4">
            ...
            <h3 className="text-right font-mono text-[12px] font-medium uppercase tracking-[0.18em] dt:text-left dt:font-serif dt:text-[clamp(1.6rem,7vw,2.25rem)] dt:font-normal dt:normal-case dt:leading-tight dt:tracking-[-0.02em]">
          ...
          <p className="mt-4 hidden max-w-[46ch] font-mono text-[13px] leading-relaxed text-ink/70 dt:mt-6 dt:block">
            {project.blurb}
          </p>
...
function Work() {
  return (
    <section id="work" className="mx-auto max-w-[1100px] px-6">
      <p className={`${LABEL} pb-5 text-center text-ink/50 dt:py-12`}>A sample of my work</p>
      {PROJECTS.map((p) => (
...
function About() {
  return (
    <section id="about" className="mx-auto max-w-[1100px] px-6 pb-14 pt-4 dt:py-20">
      <p className={`${LABEL} text-ink/50`}>About me</p>
      <div className="mt-5 grid gap-6 dt:mt-8 dt:grid-cols-12 dt:gap-8">
        <h2 className="font-serif text-[clamp(1.75rem,8vw,2.75rem)] leading-[1.1] tracking-[-0.02em] dt:col-span-5">
          {/* Kept on one line so it can't split as "mahn- / see)" */}
          {ABOUT.intro} <span className="whitespace-nowrap">{ABOUT.pronunciation}</span>
        </h2>
        <div className="space-y-5 font-mono text-[13px] leading-relaxed text-ink/75 dt:col-span-7">
          {ABOUT.body.map((para) => (
...
export default function Home() {
  return (
    <>
      ...
      <div className="rise py-8 dt:py-14" style={delay(480)}>
        <SlashRule />
      </div>

      <Work />

      <div className="py-8 dt:py-14">
        <SlashRule />
      </div>
